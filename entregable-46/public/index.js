...
socket.on("success", (data) => {
    ...
    const denormalizedData = normalizr.denormalize(data.result, fileSchema, data.entities);

    const percentageSaved = `Compresión: ${
        (JSON.stringify(data).length / JSON.stringify(denormalizedData).length) * 100
    }%`;
